  ...
  en: {
    ...
    [PAGES.FOURTH]: [
      ...
      },
      {
        ...
        name: "cleaning_bot",
        title:
          "Telegram бот для управления заказами, взаимодействия с исполнителями и заказчиками",
        description:
          "Телеграм бот для управления работами и для взаимодействия с исполнителями и заказчиками. Приложение позволяет руководителю распределять задания между исполнителями, контролировать выполнение. Исполнители могут получить информацию о назначенных им заданиях, отчитаться о выполнении, зафиксированть результат. Заказчики могут размещать заказы из католога, производить оплату, получать информацию о ходе выполнения заказа.",
        github: null,
        deploy: null,
        image: "",
        fullData: {
          ...
          architecture:
            "В основе бота лежит многоролевая архитектура, которая автоматически подстраивает интерфейс и доступные команды под руководителя, исполнителя или заказчика. Система управляет полным жизненным циклом заказа: от создания черновика и проведения оплаты через платежный шлюз до гибкого назначения ответственных исполнителей с учетом их текущей загрузки. Исполнители оперативно получают уведомления о новых задачах и могут сдавать отчеты с прикреплением фото и документов, которые мгновенно загружаются в защищенное облачное хранилище. Благодаря хранению всех сессий и состояний в высокопроизводительном кеше (Redis) бот гарантирует мгновенный отклик и легко масштабируется горизонтально для обслуживания любого количества пользователей. Встроенные механизмы разграничения прав, защиты от дублирующих действий и контроля лимитов обеспечивают полную прозрачность финансовых операций и надежное сохранение корпоративных данных на всех этапах взаимодействия.",
          images: [
            "cleaning_bot/i_1.png",
            "cleaning_bot/i_2.png",
            "cleaning_bot/i_3.png",
            "cleaning_bot/i_4.png",
            "cleaning_bot/i_5.png",
            "cleaning_bot/i_6.png",
          ],
          requirements: {
            OS: "Ubuntu 20.04+ / Linux",
            CPU: "3.3 GGz x 2",
      ...
      },
      {
        ...
        description:
          "Бот предоставляет клиенту каталог услуг, позволяет производить запись на выбранную услугу к конкретному мастеру в конкретный день с учетом загрузки и графика работы мастеров. Для управления каталогом услуг, стоимостью, данными исполнителей, графиком их работы, получения аналитической информации реализован администратиный модуль, взаимодействие с которым возможно через браузерное приложение, web app, чат мессенджера.",
        github: null,
        deploy: null,
        image: "",
        fullData: {
          ...
          architecture:
            "В основе бота лежит интеллектуальная система бронирования, которая предоставляет клиентам интерактивный каталог услуг с возможностью выбора конкретного мастера и доступного временного слота с учётом его актуальной загрузки и рабочего графика. После выбора услуги бот автоматически проверяет занятость специалистов в реальном времени и мгновенно подтверждает запись, исключая двойные бронирования и пересечения по времени. Для управления бизнес-процессами предусмотрен многофункциональный административный модуль, позволяющий редактировать каталог и цены, управлять данными мастеров и их графиками, а также просматривать детальную аналитику по записям. Взаимодействие с административной панелью реализовано через три канала: полноценное браузерное приложение для глубокого анализа, встроенный Web App для быстрых правок с мобильных устройств и непосредственно чат-интерфейс мессенджера для оперативного управления через команды. Такая гибридная архитектура обеспечивает клиентам мгновенный доступ к свободным слотам и моментальное подтверждение записи, а руководителям — полный контроль над расписанием и прозрачную статистику работы каждого мастера в режиме реального времени.",
          images: [
            "beauty_salon_bot/i_1.png",
            "beauty_salon_bot/i_2.png",
            "beauty_salon_bot/i_3.png",
